#!/usr/bin/env python3
"""
EXOSFEAR MicroGo KG — All-in-one distributed training for 6×6 Go.

Single script that runs as either COORDINATOR (full dashboard + training)
or WORKER (mini dashboard + job server). Uses Flask for both.

    pip install flask torch numpy       # full mode
    pip install flask numpy             # demo mode (mock engine, no torch)

Quick start:
    python microgo.py                   # interactive menu
    python microgo.py coordinator       # start coordinator on this machine
    python microgo.py worker            # start worker on this machine
"""
...
def get_local_ips():
    ...
    for probe in["8.8.8.8","1.1.1.1","192.168.1.1","10.0.0.1"]:
        try:
            s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM);s.connect((probe,80));add(s.getsockname()[0]);s.close()
        except:pass
    return ips
...
def get_port_pid(port):
    """Find PID using a port. Returns (pid, cmdline) or (None, None)."""
...
def normalize_worker_url(text,default_port=None):
    text=text.strip().rstrip("/")
    if not text:return""
    if not text.startswith("http://") and not text.startswith("https://"):
        if default_port and":"not in text.split("/")[-1]:text=f"http://{text}:{default_port}"
        else:text="http://"+text
    return text.rstrip("/")
...
class GoState:
    ...
    @staticmethod
    def new():
        s=GoState();s._history=frozenset([s.board_data.tobytes()]);return s
    ...
    def legal_moves(self):
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                if board[r,c]==0 and self.try_play(r*BOARD_SIZE+c)is not None:
                    moves.append(r*BOARD_SIZE+c)
        moves.append(PASS_MOVE);return moves
    def try_play(self,move):
        board=self.board_array()
        if move==PASS_MOVE:
            return GoState(board,-self.to_play,self.passes+1,self.move_count+1,self._history)
...
# ═══════════════════════════════════════════════════════════════════
#  NEURAL NET  (real if torch, stub if mock)
# ═══════════════════════════════════════════════════════════════════
if not MOCK_MODE:
    ...
    class ExpertTower(nn.Module):
        ...
        def forward(self,base):
            ...
            desc=torch.tanh(self.df(F.adaptive_avg_pool2d(h,1).flatten(1)));return logits,v,desc
    ...
    class GraphTeamNet(nn.Module):
        ...
        def forward(self,x,return_aux=False):
            ...
            phase=x[:,4].mean(dim=(1,2)).unsqueeze(-1);mob=x[:,3].mean(dim=(1,2)).unsqueeze(-1)
            turn=x[:,2].mean(dim=(1,2)).unsqueeze(-1)
            rl=self.router(torch.cat([pooled,phase,mob,turn],-1))
            pols,vals,descs=[],[],[]
            for i,exp in enumerate(self.experts):
                lo,va,de=exp(base);de=de+self.expert_token[i].unsqueeze(0)
            ...
            return fp,fv,{"weights":w,"router_probs":torch.softmax(rl,-1),"conf":conf,"expert_values":vs}
        def snapshot_graph(self):
            with torch.no_grad():
                return{"experts":list(self.expert_names),"edges":self.graph_matrix().cpu().numpy().tolist(),
    ...
    def net_from_b64(payload,device):
        net=new_team(device);raw=gzip.decompress(base64.b64decode(payload.encode("ascii")))
...
if not MOCK_MODE:
    class MCTS:
        ...
        def _eval(self,state):
            ...
            s=pr.sum();pr=mask/max(mask.sum(),1.0) if s<=0 else pr/s;return pr,val
        ...
        def run(self,root_state):
            ...
            for _ in range(self.sims):
                ...
                if state.game_over():value=1.0 if state.winner()==state.to_play else-1.0
                else:value=self._expand(node,state)
...
if not MOCK_MODE:
    ...
    def do_selfplay_job(model_b64,device,sims,games):
        net=net_from_b64(model_b64,device);results=[];packed=[];gate_means=[]
    ...
    def train_team(net,replay,device,steps=100,bs=64,lr=1e-3):
        ...
        for _ in range(steps):
            ...
            logits,value=net(x);ploss=-(p_tgt*F.log_softmax(logits,-1)).sum(-1).mean()
        ...
        net.eval()
        return{"steps":len(tl),"policy_loss":float(np.mean(pl)) if pl else None,
               "value_loss":float(np.mean(vl)) if vl else None,"total_loss":float(np.mean(tl)) if tl else None,
    ...
    def evaluate_pair(nets,device,sims,games):
        def play_match(nb,nw):
            state=GoState.new()
            while not state.game_over():
                mv=eval_move(nb if state.to_play==1 else nw,device,state,sims)
                ns=state.try_play(mv)
                if not ns:ns=state.try_play(PASS_MOVE)
                if not ns:break
                state=ns
            return state.winner(),state.final_score_black()
        wins_a=0;half=max(1,games//2)
        for _ in range(half):
            w,_=play_match(nets["A"],nets["B"])
            if w==1:wins_a+=1
        for _ in range(games-half):
            w,_=play_match(nets["B"],nets["A"])
            if w==-1:wins_a+=1
        def vs_rand(net,n=4):
            w=0
            for _ in range(n):
        ...
        avr=vs_rand(nets["A"],max(4,games//2));bvr=vs_rand(nets["B"],max(4,games//2))
        return{"games":games,"wins_A":wins_a,"wins_B":games-wins_a,"A_vs_random":avr,"B_vs_random":bvr}
    ...
    def run_one(worker,num_games):
        ...
        if worker=="__local__":
            if MOCK_MODE:
                ...
                return{"ok":True,"results":[{"winner":random.choice([1,-1])}]*num_games,
                       "samples":compress_obj([[[[0.0]*5]*6]*6,[0.0]*37,random.choice([-1.0,1.0])] for _ in range(num_games*8)),
                       "avg_gate":[0.25]*4,"worker_name":"local","seconds":round(time.time()-t0,2)}
            res=do_selfplay_job(model_b64,device,sims,num_games)
            res["worker_name"]="local";res["seconds"]=round(time.time()-t0,2);return res
        else:
            try:
                return http_json(worker.rstrip("/")+"/selfplay",method="POST",
                    payload={"model":model_b64,"sims":sims,"games":num_games},token=token,timeout=timeout)
            except Exception as e:
                # fallback to local
                if MOCK_MODE:
                    time.sleep(0.3*num_games)
                    return{"ok":True,"results":[{"winner":1}]*num_games,
                           "samples":compress_obj([]),
                           "avg_gate":[0.25]*4,"worker_name":f"fallback({worker})","seconds":round(time.time()-t0,2),"error":str(e)[:60]}
                res=do_selfplay_job(model_b64,device,sims,num_games)
                res["worker_name"]=f"fallback({worker})";res["seconds"]=round(time.time()-t0,2);return res
...
# ═══════════════════════════════════════════════════════════════════
#  WORKER JOB STATE (for worker dashboard)
# ═══════════════════════════════════════════════════════════════════
class WorkerState:
    def __init__(self):
        self.jobs_done=0;self.jobs_failed=0;self.total_games=0;self.total_samples=0
        self.busy=False;self.current_job=None;self.last_job_time=None
        self.log=deque(maxlen=200);self.started_at=now_ts()
...
class AppState:
    def __init__(self):
        ...
        self.cfg={"rounds":6,"selfplay_sims":20,"eval_sims":28,"games_per_job":2,
                  "selfplay_jobs_per_round":2,"train_steps":60,"batch_size":64,
                  "learning_rate":0.001,"eval_games":8}
        self.log=deque(maxlen=500);self.round_history=[];self.current_round=0
        self.is_training=False;self._stop_flag=False
        self.playground_state=None;self.playground_moves=[]
    def _log(self,msg):self.log.append({"ts":now_ts(),"msg":msg})
    def init_nets(self):
        for n in["A","B"]:
            self.nets[n]=new_team(self.device) if not MOCK_MODE else f"mock_{n}"
            self.replays[n]=ReplayBuffer();self.leaderboard.ensure(f"{n}_current")
        self._log("Nets initialized"+(" (demo)" if MOCK_MODE else ""))
    def get_board_json(self,state):
        board=state.board_array();cells=[]
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):cells.append({"r":r,"c":c,"v":int(board[r,c])})
        legal=state.legal_moves();legal_rc=[]
        for mv in legal:
        ...
        return{"size":BOARD_SIZE,"cells":cells,"to_play":state.to_play,
               "to_play_label":"Black" if state.to_play==1 else "White",
               "passes":state.passes,"move_count":state.move_count,"game_over":state.game_over(),
               "legal_moves":legal_rc,"score_black":state.final_score_black() if state.game_over() else None}

ST=AppState()
APP_MODE="coordinator"  # set at startup
...
# ═══════════════════════════════════════════════════════════════════
SHARED_CSS="""
:root{--bg:#0b0d0e;--bg2:#121618;--bg3:#1a1e22;--brd:#262c32;--fg:#c5cad0;--fg2:#8a9099;--fg3:#585e66;--gold:#c9a227;--gold2:#e0be4a;--teal:#3aafa9;--rose:#d45d5d;--sky:#5b8fd4;--stone-b:#181818;--stone-w:#e6e2d8;--r:5px}
*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--fg);font-family:'IBM Plex Mono',monospace;font-size:13px;line-height:1.55}a{color:var(--gold)}::selection{background:var(--gold);color:var(--bg)}
...
.top h1{font-family:'Playfair Display',serif;font-size:1.15rem;font-weight:700;color:var(--gold);white-space:nowrap}.top h1 em{font-style:normal;color:var(--fg3);font-family:'IBM Plex Mono',monospace;font-size:.7rem;font-weight:400;margin-left:.5rem;vertical-align:middle}
.tabs{display:flex}.tabs button{background:0;border:0;color:var(--fg3);font:inherit;font-size:11.5px;font-weight:500;padding:.55rem 1rem;cursor:pointer;border-bottom:2px solid transparent;transition:.12s}.tabs button:hover{color:var(--fg)}.tabs button.on{color:var(--gold);border-bottom-color:var(--gold)}
...
.c{background:var(--bg2);border:1px solid var(--brd);border-radius:var(--r);overflow:hidden;margin-bottom:1.1rem}.c:last-child{margin-bottom:0}
...
.btn{display:inline-flex;align-items:center;gap:.35rem;background:var(--gold);color:var(--bg);font:inherit;font-size:11.5px;font-weight:600;padding:6px 13px;border:0;border-radius:3px;cursor:pointer;transition:.12s}.btn:hover{background:var(--gold2)}.btn:disabled{opacity:.35;cursor:not-allowed}
...
.big-url:hover{background:rgba(201,162,39,.08)}
.step{display:flex;gap:.6rem;margin:.5rem 0;align-items:flex-start}.step-n{background:var(--gold);color:var(--bg);width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0}.step-t{font-size:12px;line-height:1.5}
"""
...
function tab(id){document.querySelectorAll('.pan').forEach(p=>p.classList.remove('on'));document.querySelectorAll('.tabs button').forEach(b=>b.classList.remove('on'));$('t-'+id).classList.add('on');document.querySelectorAll('.tabs button').forEach(b=>{if(b.textContent.toLowerCase().startsWith(id.slice(0,4)))b.classList.add('on')});if(id==='play')pNew()}
function expertBars(ew,target){if(!ew||!ew.length)return;const cols=['var(--gold)','var(--sky)','var(--teal)','var(--rose)'];const ns=['opening','tactics','territory','endgame'];$(target).innerHTML=ew.map((w,i)=>{const p=(w*100).toFixed(1);return'<div class="eb"><span class="eb-l">'+ns[i]+'</span><div class="eb-t"><div class="eb-f" style="width:'+p+'%;background:'+cols[i]+'"></div></div><span class="eb-v">'+p+'%</span></div>'}).join('')}
"""

# ═══════════════════════════════════════════════════════════════════
#  WORKER HTML TEMPLATE
# ═══════════════════════════════════════════════════════════════════
WORKER_TEMPLATE="""<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>EXOSFEAR Worker</title>
...
<div class="top"><h1>EXOSFEAR <em>Worker</em></h1><div class="top-r">
<span class="badge work">worker</span>
<span class="dot" id="sd"></span><span id="sl" style="font-size:11px;color:var(--fg3)">idle</span>
</div></div>
<div style="padding:1.25rem"><div class="g2"><div>
<div class="c"><div class="ch">Connection Info — share this with the coordinator</div><div class="cb">
<p style="font-size:12px;color:var(--fg2);margin-bottom:.4rem">The coordinator needs this URL to send jobs here:</p>
<div class="big-url" id="wurl" onclick="navigator.clipboard.writeText(this.textContent).then(()=>this.style.borderColor='var(--teal)')">loading…</div>
<p style="font-size:11px;color:var(--fg3);margin-top:.4rem">Click to copy. Token: <span id="wtok">—</span></p>
</div></div>
<div class="c"><div class="ch">Setup Instructions</div><div class="cb">
<div class="step"><span class="step-n">1</span><span class="step-t">This worker is running. Keep this terminal open.</span></div>
<div class="step"><span class="step-n">2</span><span class="step-t">On your <strong>main machine</strong>, run:<br><code style="color:var(--gold)">python microgo.py coordinator</code></span></div>
<div class="step"><span class="step-n">3</span><span class="step-t">In the coordinator's <strong>Workers</strong> tab, add the URL shown above (or use Scan LAN).</span></div>
<div class="step"><span class="step-n">4</span><span class="step-t">Start training from the coordinator dashboard. Jobs will appear here automatically.</span></div>
</div></div>
</div><div>
<div class="c"><div class="ch">Worker Status</div><div class="cb">
<table class="mt">
<tr><td>Status</td><td id="ws">idle</td></tr>
<tr><td>Device</td><td id="wdev">—</td></tr>
<tr><td>Jobs Completed</td><td id="wjobs">0</td></tr>
<tr><td>Total Games</td><td id="wgames">0</td></tr>
<tr><td>Total Samples</td><td id="wsamp">0</td></tr>
<tr><td>Last Job</td><td id="wlast">—</td></tr>
<tr><td>Up Since</td><td id="wup">—</td></tr>
</table></div></div>
<div class="c"><div class="ch">Worker Log</div><div class="cb" style="padding:0"><div class="la" id="wlog" style="height:300px"></div></div></div>
</div></div></div>
<script>"""+SHARED_JS_HELPERS+"""
async function wpoll(){try{const d=await api('/api/worker/status');
$('wurl').textContent=d.url||'—';$('wtok').textContent=d.token?'••••'+d.token.slice(-4):'none';
...
if(d.log&&d.log.length){$('wlog').innerHTML=d.log.map(e=>'<div><span class="ts">'+e.ts+'</span>'+e.msg+'</div>').join('');$('wlog').scrollTop=$('wlog').scrollHeight}
}catch(e){}}
wpoll();setInterval(wpoll,2000);
</script></body></html>"""

# ═══════════════════════════════════════════════════════════════════
#  COORDINATOR HTML TEMPLATE
# ═══════════════════════════════════════════════════════════════════
COORD_TEMPLATE="""<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>EXOSFEAR MicroGo KG</title>
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@300;400;500;600&family=Playfair+Display:wght@400;700&display=swap" rel="stylesheet"/>
<style>"""+SHARED_CSS+"""</style></head><body>
<div class="top"><h1>EXOSFEAR <em>MicroGo KG</em></h1>
<div class="tabs"><button class="on" onclick="tab('dash')">Dashboard</button><button onclick="tab('play')">Playground</button><button onclick="tab('work')">Workers</button><button onclick="tab('conf')">Config</button></div>
<div class="top-r"><span class="badge {{ 'demo' if mock else 'live' }}">{{ 'demo' if mock else 'live' }}</span><span class="dot" id="sd"></span><span id="sl" style="font-size:11px;color:var(--fg3)">idle</span></div></div>
<!-- DASHBOARD --><div class="pan on" id="t-dash"><div class="g2"><div>
<div class="c"><div class="ch">Training</div><div class="cb">
<table class="mt"><tr><td>Round</td><td id="mr">0 / —</td></tr><tr><td>Replay A</td><td id="mra">0</td></tr><tr><td>Replay B</td><td id="mrb">0</td></tr>
...
<!-- PLAYGROUND --><div class="pan" id="t-play"><div class="g2"><div>
...
<table class="mt"><tr><td>To Play</td><td id="pt">—</td></tr><tr><td>Move</td><td id="pm">0</td></tr><tr><td>Passes</td><td id="pp">0</td></tr><tr><td>Result</td><td id="pr">—</td></tr></table>
<div style="margin-top:.6rem;display:flex;gap:.4rem;flex-wrap:wrap"><button class="btn s" onclick="pNew()">New</button><button class="btn s o" onclick="pPass()">Pass</button><button class="btn s o" onclick="pAI()">AI Move</button><button class="btn s o" onclick="pAuto()">Autoplay</button></div></div></div>
</div><div>
<div class="c"><div class="ch">Expert Analysis</div><div class="cb" id="pex"><p class="em">Start a game</p></div></div>
<div class="c"><div class="ch">Moves</div><div class="cb" id="pmv"><p class="em">No moves</p></div></div>
</div></div></div>
<!-- WORKERS --><div class="pan" id="t-work"><div class="g2"><div>
...
<div class="step"><span class="step-n">1</span><span class="step-t">On each worker machine, run:<br><code style="color:var(--gold)">python microgo.py worker</code></span></div>
<div class="step"><span class="step-n">2</span><span class="step-t">Copy the worker URL and paste it above, or click <strong>Scan LAN</strong>.</span></div>
<div class="step"><span class="step-n">3</span><span class="step-t">Go to <strong>Dashboard</strong> and click <strong>Start</strong>.</span></div>
</div></div>
</div><div>
<div class="c"><div class="ch">Cluster</div><div class="cb"><table class="mt">
<tr><td>Device</td><td id="wd">—</td></tr><tr><td>Local IPs</td><td id="wi">—</td></tr><tr><td>Local Worker</td><td id="wlc">—</td></tr><tr><td>Token</td><td id="wt">—</td></tr>
</table></div></div></div></div></div>
<!-- CONFIG --><div class="pan" id="t-conf"><div class="g2"><div>
<div class="c"><div class="ch">Training Parameters</div><div class="cb">
...
<label class="fld">Learning rate<input type="text" id="cl" value="0.001"/></label>
...
<button class="btn" onclick="saveCfg()" style="margin-top:.4rem">Save</button></div></div>
</div><div>
<div class="c"><div class="ch">Cluster</div><div class="cb">
<label class="fld">Token<input type="text" id="ctk" value=""/></label>
<label class="fld">Local Worker<select id="clo"><option value="1" selected>Yes</option><option value="0">No</option></select></label>
<button class="btn" onclick="saveClus()" style="margin-top:.4rem">Save</button></div></div>
<div class="c"><div class="ch">Actions</div><div class="cb" style="display:flex;flex-direction:column;gap:.4rem">
<button class="btn o" onclick="resetN()">Reset Networks</button><button class="btn o" onclick="expSt()">Export</button></div></div>
</div></div></div>
<script>"""+SHARED_JS_HELPERS+"""
...
if(d.leaderboard&&d.leaderboard.length)$('lbd').innerHTML=d.leaderboard.map((e,i)=>'<div class="lr"><span class="lr-k">'+(i+1)+'</span><span class="lr-n">'+e[0]+'</span><span class="lr-e">'+Math.round(e[1])+'</span></div>').join('');
expertBars(d.expert_weights,'ebs');
if(d.graph&&d.graph.edges){const ns=d.graph.experts,n=ns.length;let h='<div class="ag" style="grid-template-columns:58px repeat('+n+',46px)"><div></div>';ns.forEach(nm=>{h+='<div class="ah">'+nm.slice(0,4)+'</div>'});d.graph.edges.forEach((row,ri)=>{h+='<div class="ah" style="text-align:right;padding-right:3px">'+ns[ri].slice(0,4)+'</div>';row.forEach(v=>{const a=Math.min(1,v*2);h+='<div class="ac" style="background:rgba(201,162,39,'+a.toFixed(2)+')">'+v.toFixed(2)+'</div>'})});h+='</div>';$('adj').innerHTML=h}
if(d.log&&d.log.length){const el=$('elog');el.innerHTML=d.log.map(e=>'<div><span class="ts">'+e.ts+'</span>'+e.msg+'</div>').join('');el.scrollTop=el.scrollHeight}
if(d.round_history&&d.round_history.length){const cv=$('lc'),ctx=cv.getContext('2d');const dpr=devicePixelRatio||1;cv.width=cv.offsetWidth*dpr;cv.height=170*dpr;ctx.scale(dpr,dpr);const w=cv.offsetWidth,h=170;ctx.clearRect(0,0,w,h);const la=d.round_history.map(r=>r.loss_a).filter(v=>v!=null),lb=d.round_history.map(r=>r.loss_b).filter(v=>v!=null);if(la.length){const all=[...la,...lb],mn=Math.min(...all),mx=Math.max(...all),rng=mx-mn||1;const pad={t:8,r:8,b:20,l:34},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b;ctx.strokeStyle='#262c32';ctx.lineWidth=1;for(let i=0;i<=4;i++){const y=pad.t+ph*i/4;ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke()}function line(data,col){ctx.strokeStyle=col;ctx.lineWidth=2;ctx.beginPath();data.forEach((v,i)=>{const x=pad.l+(i/Math.max(1,data.length-1))*pw,y=pad.t+(1-(v-mn)/rng)*ph;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y)});ctx.stroke()}line(la,'#c9a227');line(lb,'#5b8fd4');ctx.fillStyle='#585e66';ctx.font='10px IBM Plex Mono';ctx.fillText(mx.toFixed(3),1,pad.t+8);ctx.fillText(mn.toFixed(3),1,h-pad.b-2)}}
}catch(e){}}
...
async function expSt(){const d=await api('/api/export');const b=new Blob([JSON.stringify(d,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='microgo_ckpt.json';a.click()}
...
async function rmW(u){await api('/api/workers/remove',{url:u});rfW()}
...
async function rfW(){const d=await api('/api/workers');$('wd').textContent=d.device;$('wi').textContent=(d.local_ips||[]).join(', ')||'—';$('wlc').textContent=d.include_local?'Yes':'No';$('wt').textContent=d.token?'••••'+d.token.slice(-4):'none';const el=$('wl');if(!d.workers||!d.workers.length){el.innerHTML='<p class="em">No workers — add one above or Scan LAN</p>';return}el.innerHTML=d.workers.map(w=>'<div class="wr"><span class="dot '+(w.ok?'ok':'')+'"></span><span class="u">'+w.url+'</span><span class="ws '+(w.ok?'ok':'er')+'">'+(w.ok?'online':(w.error||'offline'))+'</span><button class="btn s d" onclick="rmW(\''+w.url+'\')">✕</button></div>').join('')}
...
async function pMv(m){const d=await api('/api/playground/move',{move:m});rB(d)}
...
b.cells.forEach(c=>{if(c.v===0)return;const cx=mg+c.c*cs,cy=mg+c.r*cs,rd=cs*.41;if(c.v===1){h+='<circle cx="'+cx+'" cy="'+cy+'" r="'+rd+'" fill="#181818" stroke="#2a2a2a" stroke-width=".5"/><circle cx="'+(cx-rd*.22)+'" cy="'+(cy-rd*.22)+'" r="'+(rd*.16)+'" fill="#333" opacity=".45"/>'}else{h+='<circle cx="'+cx+'" cy="'+cy+'" r="'+rd+'" fill="#e6e2d8" stroke="#bbb" stroke-width=".5"/><circle cx="'+(cx-rd*.22)+'" cy="'+(cy-rd*.22)+'" r="'+(rd*.16)+'" fill="#fff" opacity=".55"/>'}});
if(!b.game_over)b.legal_moves.forEach(lm=>{if(lm.r<0)return;const cx=mg+lm.c*cs,cy=mg+lm.r*cs;const hit=b.cells.find(c=>c.r===lm.r&&c.c===lm.c);if(hit&&hit.v!==0)return;h+='<circle cx="'+cx+'" cy="'+cy+'" r="'+(cs*.28)+'" fill="transparent" stroke="'+(b.to_play===1?'#444':'#ccc')+'" stroke-width="1" stroke-dasharray="3,3" opacity="0" cursor="pointer" onmouseover="this.setAttribute(\'opacity\',\'.45\')" onmouseout="this.setAttribute(\'opacity\',\'0\')" onclick="pMv('+lm.move+')"/>'});
...
expertBars(data.experts,'pex');
if(data.moves&&data.moves.length)$('pmv').innerHTML='<div class="ml">'+data.moves.map((m,i)=>'<span class="mc '+(i%2===0?'b':'w')+'">'+(i+1)+'.'+m+'</span>').join('')+'</div>';
else $('pmv').innerHTML='<p class="em">No moves</p>'}
...
@app.route("/")
def index():
    if APP_MODE=="worker":return render_template_string(WORKER_TEMPLATE)
    return render_template_string(COORD_TEMPLATE,mock=MOCK_MODE)

@app.route("/health")
def health():
    return jsonify({"ok":True,"role":"microgo_worker","worker_name":socket.gethostname(),
                    "device":ST.device,"version":VERSION,"pid":os.getpid()})

# ═══════════════════════════════════════════════════════════════════
#  ROUTES — worker selfplay endpoint (accepts jobs from coordinator)
# ═══════════════════════════════════════════════════════════════════
@app.route("/selfplay",methods=["POST"])
def selfplay_endpoint():
    ...
    WORKER_STATE.busy=True;WORKER_STATE.current_job=now_ts()
    WORKER_STATE._log(f"Job received: {data.get('games',0)} games, {data.get('sims',0)} sims")
    t0=time.time()
    try:
        if MOCK_MODE:
            games=int(data.get("games",2));sims=int(data.get("sims",20))
            time.sleep(0.4*games)
            res={"ok":True,"results":[{"winner":random.choice([1,-1])}]*games,
                 "samples":compress_obj([]),
                 "avg_gate":[0.25]*4,"worker_name":socket.gethostname(),
                 "seconds":round(time.time()-t0,2)}
            WORKER_STATE.jobs_done+=1;WORKER_STATE.total_games+=games
            WORKER_STATE._log(f"Job done: {games} games in {res['seconds']:.1f}s (mock)")
        else:
            res=do_selfplay_job(model_b64=data["model"],device=ST.device,
                                sims=int(data.get("sims",20)),games=int(data.get("games",2)))
            res["ok"]=True;res["worker_name"]=socket.gethostname();res["seconds"]=round(time.time()-t0,2)
            ng=len(res.get("results",[]));ns=len(decompress_obj(res["samples"])) if res.get("samples") else 0
            WORKER_STATE.jobs_done+=1;WORKER_STATE.total_games+=ng;WORKER_STATE.total_samples+=ns
            WORKER_STATE._log(f"Job done: {ng} games, {ns} samples in {res['seconds']:.1f}s")
        WORKER_STATE.last_job_time=now_ts()
        return jsonify(res)
    except Exception as e:
        WORKER_STATE.jobs_failed+=1;WORKER_STATE._log(f"Job FAILED: {e}")
...
def worker_status():
    ...
    return jsonify({"url":url,"device":ST.device,"token":app.config.get("AUTH_TOKEN",""),
                    "busy":WORKER_STATE.busy,"jobs_done":WORKER_STATE.jobs_done,
                    "jobs_failed":WORKER_STATE.jobs_failed,"total_games":WORKER_STATE.total_games,
                    "total_samples":WORKER_STATE.total_samples,"last_job_time":WORKER_STATE.last_job_time,
                    "started_at":WORKER_STATE.started_at,"log":list(WORKER_STATE.log)})
...
def api_status():
    ...
    return jsonify({"is_training":ST.is_training,"current_round":ST.current_round,"cfg":ST.cfg,"mock":MOCK_MODE,
        "replay_a":ST.replays["A"].size() if"A"in ST.replays else 0,
        "replay_b":ST.replays["B"].size() if"B"in ST.replays else 0,
        "last_loss_a":last.get("loss_a"),"last_loss_b":last.get("loss_b"),
        "a_vs_random":last.get("a_vs_random"),"b_vs_random":last.get("b_vs_random"),"h2h":last.get("h2h"),
        "expert_weights":last.get("expert_weights"),"expert_names":list(EXPERT_NAMES),
        "graph":last.get("graph"),"leaderboard":ST.leaderboard.top()[:10],
        "round_history":ST.round_history,"log":list(ST.log)})
...
def api_workers():
    return jsonify({"workers":ping_workers(ST.worker_urls,ST.token) if ST.worker_urls else[],
        "device":ST.device,"local_ips":get_local_ips(),"include_local":ST.include_local,"token":ST.token})

@app.route("/api/workers/add",methods=["POST"])
def wk_add():
    url=(request.json or{}).get("url","").strip().rstrip("/")
    if url and url not in ST.worker_urls:ST.worker_urls.append(url);ST._log(f"Worker added: {url}")
    return jsonify({"ok":True})

@app.route("/api/workers/remove",methods=["POST"])
def wk_rm():
    url=(request.json or{}).get("url","").strip().rstrip("/")
    ST.worker_urls=[w for w in ST.worker_urls if w!=url];return jsonify({"ok":True})
...
def wk_scan():
    if MOCK_MODE:ST._log("Scan: mock");return jsonify({"ok":True,"found":0})
    found=[];
    ...
    ST._log(f"Scan: {len(found)} found, {len(new_urls)} new");return jsonify({"ok":True,"found":len(found),"new":new_urls})
...
def api_export():
    ...
    if not MOCK_MODE:
        for n in["A","B"]:
            if n in ST.nets:out[f"model_{n}"]=net_to_b64(ST.nets[n])
    return jsonify(out)

@app.route("/api/train/start",methods=["POST"])
def train_start():
    if ST.is_training:return jsonify({"ok":False,"error":"already training"})
    if not ST.nets:ST.init_nets()
    ST.is_training=True;ST._stop_flag=False
    threading.Thread(target=_train_loop,daemon=True).start();return jsonify({"ok":True})
...
def pg_new():
    ST.playground_state=GoState.new();ST.playground_moves=[]
    return jsonify({"board":ST.get_board_json(ST.playground_state),"moves":[],"experts":_gexp(ST.playground_state),"expert_names":list(EXPERT_NAMES)})
...
def pg_move():
    ...
    return jsonify({"board":ST.get_board_json(ST.playground_state),"moves":ST.playground_moves,"experts":_gexp(ST.playground_state),"expert_names":list(EXPERT_NAMES)})

@app.route("/api/playground/ai",methods=["POST"])
def pg_ai():
    if not ST.playground_state:ST.playground_state=GoState.new();ST.playground_moves=[]
    if ST.playground_state.game_over():return jsonify({"board":ST.get_board_json(ST.playground_state),"moves":ST.playground_moves,"experts":None,"expert_names":list(EXPERT_NAMES)})
...
def _gexp(state):
    if MOCK_MODE:
        phase=state.move_count/MAX_GAME_LEN;w=[max(.05,.4-phase*.35),max(.05,.2+phase*.15),max(.05,.2+phase*.1),max(.05,.2+phase*.2)]
...
def _train_loop():
    ...
    except Exception as e:ST._log(f"Error: {e}")
...
def _mock_train():
    ...
    for ri in range(1,rounds+1):
        ...
        ST.current_round=ri;ST._log(f"═══ Round {ri}/{rounds} (demo) ═══")
        for n in["A","B"]:
            ST._log(f"  Self-play {n} (parallel dispatch)")
            results=push_selfplay_parallel("mock",ST.cfg["selfplay_sims"],
                ST.cfg["selfplay_jobs_per_round"]*ST.cfg["games_per_job"],
                ST.worker_urls,ST.token,ST.include_local,ST.device)
            for res in results:
                wn=res.get("worker_name","?");ws=res.get("seconds",0)
                ST._log(f"    {wn}: {len(res.get('results',[]))}g {ws:.1f}s")
                ST.replays[n].add([None]*len(res.get("results",[])))
        la=max(.3,4.5-ri*.4+random.uniform(-.2,.2));lb=max(.3,4.6-ri*.38+random.uniform(-.2,.2))
        ST._log(f"  Train A:{la:.4f} B:{lb:.4f}")
        phase=ri/rounds;ew=[max(.05,.4-phase*.35),max(.05,.2+phase*.15),max(.05,.2+phase*.1),max(.05,.2+phase*.2)]
        s=sum(ew);ew=[w/s for w in ew]
        ...
        ST.round_history.append({"round":ri,"loss_a":round(la,4),"loss_b":round(lb,4),
            "a_vs_random":round(avr,3),"b_vs_random":round(bvr,3),"h2h":f"{wa}/8",
            "expert_weights":ew,"graph":{"experts":list(EXPERT_NAMES),"edges":edges,"temp":1.0-ri*.05}})
        ST._log(f"  Eval: A wins {wa}/8")
        ST.leaderboard.ensure(f"A_r{ri:03d}",ST.leaderboard.ratings.get("A_current",1000))
        ST.leaderboard.ensure(f"B_r{ri:03d}",ST.leaderboard.ratings.get("B_current",1000))
    ST._log("Training complete")

def _real_train():
    cfg=ST.cfg;rounds=cfg["rounds"];sims=cfg["selfplay_sims"];device=ST.device
    total=cfg["selfplay_jobs_per_round"]*cfg["games_per_job"]
    for ri in range(1,rounds+1):
        if ST._stop_flag:ST._log("Stopped");break
        ST.current_round=ri;ST._log(f"═══ Round {ri}/{rounds} ═══")
        for nn in["A","B"]:
            ST._log(f"Self-play {nn} ({total} games, parallel dispatch)")
            mb=net_to_b64(ST.nets[nn])
            results=push_selfplay_parallel(mb,sims,total,ST.worker_urls,ST.token,ST.include_local,device)
            for res in results:
                if res.get("samples"):
                    ...
                    ST.replays[nn].add(samps)
                wn=res.get("worker_name","?");ws=res.get("seconds",0)
                ST._log(f"  {nn}·{wn}: {len(res.get('results',[]))}g {ws:.1f}s")
        ST._log("Training...");info={"round":ri}
        for n in["A","B"]:
            tr=train_team(ST.nets[n],ST.replays[n],device,cfg["train_steps"],cfg["batch_size"],cfg["learning_rate"])
            info[f"loss_{n.lower()}"]=tr.get("total_loss")
            if tr["total_loss"]:ST._log(f"  {n}: loss={tr['total_loss']:.4f}")
            if tr.get("graph"):info["graph"]=tr["graph"]
        ST._log("Eval...");ev=evaluate_pair(ST.nets,device,cfg["eval_sims"],cfg["eval_games"])
        sa=ev["wins_A"]/max(1,ev["games"]);ST.leaderboard.update("A_current","B_current",sa)
        info["a_vs_random"]=ev.get("A_vs_random");info["b_vs_random"]=ev.get("B_vs_random")
        info["h2h"]=f"{ev['wins_A']}/{ev['games']}"
        try:_,_,aux=infer_aux(ST.nets["A"],device,GoState.new());info["expert_weights"]=aux["weights"]
        except:pass
        ST.round_history.append(info);ST._log(f"  A wins {ev['wins_A']}/{ev['games']}")
        ST.leaderboard.ensure(f"A_r{ri:03d}",ST.leaderboard.ratings.get("A_current",1000))
        ST.leaderboard.ensure(f"B_r{ri:03d}",ST.leaderboard.ratings.get("B_current",1000))
    ST._log("Training complete")
...
# ═══════════════════════════════════════════════════════════════════
def banner(lines):
    w=max(len(l) for l in lines)+4;print("╔"+"═"*w+"╗")
...
def run_coordinator(port=DEFAULT_COORD_PORT,token="",device=None,interactive=True):
    ...
    ips=get_local_ips()
    ip_lines=[f"            http://{ip}:{port}" for ip in ips]
    tok_display=token[:8]+"..." if len(token)>8 else(token or"none")
    banner([
        "EXOSFEAR MicroGo KG — COORDINATOR","",
        f"Dashboard:  http://localhost:{port}",
        *ip_lines,
        f"Device:     {ST.device}",
        f"Mode:       {'DEMO (no torch)' if MOCK_MODE else 'LIVE'}",
        f"Token:      {tok_display}","",
        "Next steps:",
        f"  1. Open http://localhost:{port} in your browser",
        "  2. Go to Workers tab -> add worker URLs or Scan LAN",
        "  3. Click Start on the Dashboard tab",
    ])
...
def run_worker(port=DEFAULT_WORKER_PORT,token="",device=None,interactive=True):
    ...
    url_lines=[f"  -> {u}" for u in urls] if urls else[f"  -> http://<this-ip>:{port}"]
    tok_cmd=f" --token {token}" if token else""
    banner([
        "EXOSFEAR MicroGo KG — WORKER","",
        "Share this URL with the coordinator:",
        *url_lines,"",
        f"Device:   {ST.device}",
        f"Token:    {token[:8]+'...' if len(token)>8 else token or 'none'}","",
        "This worker is ready. On the coordinator machine:",
        f"  1. python microgo.py coordinator{tok_cmd}",
        "  2. Add this URL in the Workers tab",
        "  3. Start training",
    ])
    WORKER_STATE._log(f"Worker started on port {port}")
    WORKER_STATE._log(f"Device: {ST.device}, URLs: {', '.join(urls)}")
    app.config["AUTH_TOKEN"]=token;app.config["WORKER_PORT"]=port
    app.run(host="0.0.0.0",port=port,debug=False,use_reloader=False)
...
def interactive_menu():
    ...
    print("  ║                                          ║")
    if MOCK_MODE:
        print("  ║   ⚠  torch not found — demo mode        ║")
    else:
        print("  ║   ✓  torch found — full mode             ║")
    print("  ╚══════════════════════════════════════════╝")
    print()
    try:choice=input("  Select [1]: ").strip() or"1"
    except(EOFError,KeyboardInterrupt):choice="1"

    if choice in("1","coordinator","c"):
        token=secrets.token_urlsafe(8)
        try:
            raw=input(f"  Shared token [Enter={token}]: ").strip()
            if raw:token=raw
            if raw.lower() in("none","no"):token=""
        except:pass
        run_coordinator(token=token)
    elif choice in("2","worker","w"):
        token=""
        try:token=input("  Shared token (must match coordinator) [none]: ").strip()
        except:pass
...
def main():
    parser=argparse.ArgumentParser(description="EXOSFEAR MicroGo KG — All-in-one")
    parser.add_argument("mode",nargs="?",default="menu",choices=["menu","coordinator","worker"])
    ...
    if args.mode=="coordinator":
        run_coordinator(port=args.port or DEFAULT_COORD_PORT,token=args.token,device=args.device,interactive=args.port is None)
    elif args.mode=="worker":
        run_worker(port=args.port or DEFAULT_WORKER_PORT,token=args.token,device=args.device,interactive=args.port is None)
    else:
        interactive_menu()
